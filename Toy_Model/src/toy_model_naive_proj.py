...
y_EM = np.zeros(len(phi_vector), dtype=np.complex_)
...
for l in range(0, numtsteps + 1):
  ...
  # 1. Take EM step 
  # 2. evaluate gradient 
  # 3. Solve for \psi that gets you back on the manifold  --> can be done analytically because of linear constraint: 
  #    --> \psi = -(sum(y)) / 3 
  ...
  # Calculate psi ( a scalar) 
  psi_iter = 0.
  psi_iter = -np.sum(y_EM)/3.

  # step y_tilde to y_lp1
