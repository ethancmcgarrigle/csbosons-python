...
init_guess = np.zeros(len(d_vector), dtype=np.complex_) # size of DOF + num constraints (only 1)
...
io_functions.write_observables("data_projected_grad.dat", observables, t, True)

opout=open('operators_psi_proj_grad.dat',"w")
...
for l in range(0, numtsteps + 1):
  ...
  for i in range(0, max_iterations + 1):
    ...
    # do Ax = b, using G as A and delta as b ### ALTERNATIVE: USE GRADIENT DESCENT, important for larger systems 
    solution = grad_solver.grad_descent(G_matrix, d_vector, init_guess, stepsize, max_iterations, ETOL, print_flag)
  ...
  if(l % iofreq == 0 and l > 0):
    ...
    observables = [constraint_avg, Tr_U_avg, Tr_Usq_avg, Tr_U_Udag_avg, rho_avg]
    io_functions.write_observables("data_projected_grad.dat", observables, t, False)
    num_iters_s[ctr] = num_iters_avg 

    opout=open('operators_psi_proj_grad.dat',"a")
