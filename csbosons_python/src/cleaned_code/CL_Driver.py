...
class CL_Driver:
  ...
  # Constructor   
  def  __init__(self, Model, CLnoise, isOffDiagonal, ETD, do_implicit, isShifting, dt, numtsteps, iofreq, num_samples): 
    # initialize all the system vars 
    self.CLnoise = CLnoise
    self.isOffDiagonal = isOffDiagonal 
    self.ETD = ETD 
  ...
  def fill_tstep_coefs(self):
    ...
    for j in range(0, self.ntau):
      self.lin_tstep_coef[:,j] = self.model.lincoef[:, j] + self.model.Bn[:, j]
      ...
      for m in range(0, self.N_spatial): 
        if(self.lin_tstep_coef[m, j] == 0.):
          ...
          #self.noisescl[m, j] = np.sqrt(ntau * dt / dV)
          #self.noisescl_phistar[m, j] = np.sqrt(ntau * dt / dV)
        else: 
          ...
          self.noisescl[m, j] = np.sqrt((1. - np.exp(-2. * self.lin_tstep_coef[m, j] * self.ntau * self.dt))/(2. * self.lin_tstep_coef[m, j]))
          self.noisescl_phistar[m, j] = np.sqrt((1. - np.exp(-2. * self.linstar_tstep_coef[m, j] * self.ntau * self.dt))/(2. * self.linstar_tstep_coef[m, j]))
  ...
  def run_simulation(self, _isPlotting):
    ...
    for l in range(0, self.numtsteps + 1):
      ...
      # Step the constraint field/scalar if doing CANONICAL ensemble 
#     TODO if(self.model.ensemble == 'CANONICAL'):
#        self.step_psi()
 ...
 #         if(self.model.ensemble == "CANONICAL"):
 #           psi_s[ctr] = self.model.psi
  ...
  def timestep(self):
      ...
      if(not _ETD):
        ...
        # Do EM step 
        if(_do_implicit):
          self.model.phi, self.model.phistar = Timesteppers.EM_implicit(self.model.phi, self.model.phistar, self.model.dSdphistar, self.model.dSdphi, self.isOffDiagonal, self.CLnoise, 1., self.dt, tmp, tmpstar)
        else:
          self.model.phi, self.model.phistar = Timesteppers.EM(self.model.phi, self.model.phistar, self.model.dSdphistar, self.model.dSdphi, self.isOffDiagonal, self.CLnoise, 1., self.dt)
      else:  
        if(_do_implicit): 
          self.model.phi, self.model.phistar = Timesteppers.ETD_implicit(self.model.phi, self.model.phistar, self.model.dSdphistar, self.model.dSdphi, self.lin_tstep_coef, self.linstar_tstep_coef, self.nonlincoef, self.nonlincoef_phistar, self.noisescl, self.noisescl_phistar, self.CLnoise, tmp, self.isShifting, self.dt)
        else:
  ...
  def plot_results(self, ctr):  
    ...
    plt.plot(self.t_s[0:ctr], self.model.N_operator.samples[0:ctr].real, '*-', color = 'green', linewidth = 0.5, label = 'Samples: real')
    plt.plot(self.t_s[0:ctr], self.model.N_operator.samples[0:ctr].imag, '*-', color = 'skyblue', linewidth=0.5,label = 'Samples: imag')
    ...
    plt.plot(self.t_s[0:ctr], self.model.N_operator.samplesSq[0:ctr].real, '*-', color = 'purple', label = 'Samples')
    plt.plot(self.t_s[0:ctr], self.model.N_operator.samplesSq[0:ctr].imag, '*-', color = 'skyblue', label = 'Samples')
    # plt.plot(t_s, np.ones(len(t_s)), 'k', label = 'Constraint')
    plt.xlabel('CL time', fontsize = 20, fontweight = 'bold')
