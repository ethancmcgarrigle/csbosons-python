...
_beta1, _beta2, _mu, _h, dt, numtsteps, iofreq, seed = parameters
np.random.seed(seed)
...
for l in range(0, numtsteps + 1):
  ...
  # Solve for the psi that gives the correct constraint 
  # Sum{phi^{n+1}} = 0     ==>     Sum{ EM(phi_i) } + 3i psi = 0 
  # psi = -1/3 * (Sum { EM(phi_i) } ) 
  _psi = (1j/3.) * np.sum(phi_vector)
...
io_functions.write_observables("data_biased.dat", observables, t)

# plots 
io_functions.plot_CL_traces(t_s, observables)

observables.append(psi_s)
